...
# p <- ggplot(df_all, aes(x = time, y = msd, colour = type)) +
#   geom_line(size = 0.85)+
...
# Zoom 2: [4, 5]
p_zoom2 <- ggplot(subset(df_all, time >= 5.5 & time <= 6.5),
                  aes(time, msd, colour = type)) +
  geom_line(size = 0.7) +
  scale_colour_manual(values = c("black", "#33608CFF", "#B81840FF", "#F6BA57FF")) +
  theme_bw(base_size = 9) +
...
# Top zoom box rectangle (TALLER NOW)
xmin1 <- 0.05 * Tmax
xmax1 <- 0.45 * Tmax
ymin1 <- 0.58 * ymax
ymax1 <- 0.99 * ymax   # <-- TALLER BOX

# Bottom zoom box rectangle (unchanged)
xmin2 <- 0.55 * Tmax
xmax2 <- 0.95 * Tmax
ymin2 <- 0.002 * ymax
ymax2 <- 0.4 * ymax
...
annotation_custom(
  ...
  # diagonal connectors (top) to BOTTOM corners
  geom_segment(
    aes(
      x = 0.0,
...
annotation_custom(
  ...
  # diagonal connectors (bottom) to UPPER corners
  geom_segment(
    aes(
      x = 5.5,
      y = df_all$msd[which.min(abs(df_all$time - 4))],
      xend = xmin2,
      yend = ymax2     # <-- upper-left corner
    ),
    linewidth = 0.4, linetype = 2
  ) +
  geom_segment(
